...
ipcMain.handle('launch-game', async (_event, { platform, romPath, emulator, launchPath, controllerInfo }) => {
  ...
  // Vor dem Emulator-Start die Controller-Konfiguration vorbereiten,
  // sodass der im Launcher erkannte Controller auch im Spiel funktioniert.
  if (controllerInfo) {
    try {
      ...
      if (result.applied) {
        console.log(`[ControllerSetup] ${result.profile} konfiguriert: ${result.path}`);
      } else {
        console.warn(`[ControllerSetup] Nicht angewendet: ${result.reason}`);
      }
    } catch (err) {
      console.warn(`[ControllerSetup] Fehler: ${err.message}`);
    }
  }
